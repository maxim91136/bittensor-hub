/**
 * Top Subnets History API
 * Returns historical snapshots of Top 10 Subnets rankings.
 * 
 * Query params:
...
export async function onRequest(context) {
  ...
  try {
    const raw = await KV.get('top_subnets_history');
    if (!raw) {
      return new Response(JSON.stringify({ 
        error: 'No subnets history found', 
        _source: 'top_subnets_history', 
        _status: 'empty',
        history: []
      }), {
        status: 200,
    ...
    return new Response(JSON.stringify({ error: 'Failed to fetch subnets history', details: e.message }), {
      status: 500,
      headers: cors
    });
  }
