/**
 * Top Validators History API
 * Returns historical snapshots of Top 10 Validators rankings.
 * 
 * Query params:
...
export async function onRequest(context) {
  ...
  try {
    const raw = await KV.get('top_validators_history');
    if (!raw) {
      return new Response(JSON.stringify({ 
        error: 'No validators history found', 
        _source: 'top_validators_history', 
        _status: 'empty',
        history: []
      }), {
        status: 200,
    ...
    return new Response(JSON.stringify({ error: 'Failed to fetch validators history', details: e.message }), {
      status: 500,
      headers: cors
    });
  }
