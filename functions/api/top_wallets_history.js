/**
 * Top Wallets History API
 * Returns historical snapshots of Top 10 Wallets rankings.
 * 
 * Query params:
...
export async function onRequest(context) {
  ...
  try {
    const raw = await KV.get('top_wallets_history');
    if (!raw) {
      return new Response(JSON.stringify({ 
        error: 'No wallets history found', 
        _source: 'top_wallets_history', 
        _status: 'empty',
        history: []
      }), {
        status: 200,
    ...
    return new Response(JSON.stringify({ error: 'Failed to fetch wallets history', details: e.message }), {
      status: 500,
      headers: cors
    });
  }
